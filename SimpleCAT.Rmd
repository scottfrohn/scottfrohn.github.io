...
At a high-level, here are the steps for implementing an IRT-based CAT (adapted from Magis, Yan, von Davier, 2017)[@magis2017cat]
...
1.  Initial Step (step 2 above)
2.  Test Step (steps 3:10)
3.  Stopping Step (steps 7, 11, 12)
...
Let's imagine we answered the first item incorrectly (response = 0). Item parameters c(a, b, c) are c(1, .5, .1).
...
To do this, the function `est_ability_mle` has a (default) argument to employ an adjustment factor (aka kludge) of $\frac{1}{3\sqrt{n}}$ to each item, where $n$ is the number of items in the response vector. For instance, the response vector `c(0,0,0,0)` would have an adjustment factor of $\frac{1}{3\sqrt{4}} = \frac{1}{6}$ and therefore the vector would become `c(0.167,0.167,0.167,0.167)`.
...
### 8. Select the next item from eligible items.
