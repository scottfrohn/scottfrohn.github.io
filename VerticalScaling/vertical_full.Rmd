...
- **Item Response Theory (IRT)**: Vertical scaling often employs IRT models to account for differences in item difficulty and student ability across grades.
...
#### Prepare Workspace 
First, load the relevant packages, including custom package `CATFunctions` to support the MLE estimation. 
...
Create function to use parallel processing for efficient ability estimation during simulation
```{r}
...
### The Dataset{.tabset}
**KB04 / Kolen - Brennan (2004)**: This (unidimensional) dataset includes three parameter logistic model (3PL) item parameter estimates for two groups. Both forms were calibrated separately. The Form X items are from a "new" form and the Form Y items are from an "old" form. The data are listed in Table 6.5 in both the 2nd (2004) and 3rd (2014) edition. There are 12 common items between the two forms. We can load the `KB04` dataset from `plink`. 
...
Now that we've confirmed the scaling results, let's simulate abilities and check the impact of scaling.

#### Simulate Ability

Step 1: Simulate ability levels

Two groups, and group 1 is lower ability than group 2
...
Step 2: Generate response data based on our 3pl model
...
-   `x.new` are the simulated Form X response data, with the New Form X item parameters
-   `y` are the simulated Form Y response data, with the Form Y item parameters


Now let's estimate the abilities, the standard error, and percentile rank for our test-takers. 
```{r}
...
#### Visualize Scores{.tabset}

As you can see, the x.old and y groups have a similar distribution of scores, which makes sense. The abilities were simulated along a normal distribution based on item parameters which were calibrated to be item-centered (mean of ~zero). When we simulate response patterns based on normally distributed true abilities and item parameters centered at 0, we would expect to have similar estimated ability distributions. Once we rescaled the item parameters for Form X (x.new) and estimated abilities from the same response patterns as before, we can see that the distribution shifts: 

- It's more spread out, which checks out, since the Scaling Summary section showed the SD of the b-parameters went from 1.099855 to 1.293511, and
- It's shifted to the left, which also makes sense, since the mean of the b-parameters went from 0.810591 to 0.449127. 
...
#### Visualize Density, Info, and SE{.tabset}
...
Now that we've completed vertical scaling, and estimated abilities for our test takers, we should convert the ability estimates (in logits) to some other scale.. This process is called  **scaling** and it helps score interpretation and tracking for our audience. 

Let's look at the range of our two groups, x.new and y.
...
We have quite the range here, with some extreme values for the x.new group (min = -12.11), and even for the y group (min = -5.78). There are both linear and non-linear methods we could employ to convert these ability estimates to another scale.
...
However, because we have some outliers on the low end of the latent ability scale ($\theta_{\text{min}} = -12.11$), the scaled scores in the middle distribution will be relatively narrow. Let's look at the scale range for quintiles:
```{r}
...
This shows that the scale range for the bottom 20% of our sample spans 398 scaled points, while the middle fifth (40%-60%) is separated by a mere 38 points. That's a lot of separation for test takers for which we don't have a lot of precision, and very little separation for a group of test-takers for which we do have a lot of measurement precision. 
...
That looks a lot more reasonable, with the middle quintiles each having a spread of around 80 to 100 points. Let's try one more linear scaling option.
...
scaled.scores %>%
  ...
  theme_clean() +
  theme(legend.position.inside = c(0.95, 0.05),  # Position in the top left corner
        legend.position = "inside",
...
The linear piecewise scaling approach seems to give us the best spread for the middle quintiles, though the downside is we artificially limit (remove) variability beyond the bounds we set (everyone below -4 logits has the same scaled score)
...
#### Simulate Ability
Now, let's simulate ability levels as if we are tracking the same group of 1000 students longitudinally. To do this we'll estimate g3 ability, and apply a base growth rate with some jitter, then standardize the abilities for each grade. 
...
for(i in 1:length(grades)) {
  ...
  abilities <- tk07.abilities %>% filter(group == grades[i])
  abilities.old <- tk07.abilities.old %>% filter(group == grades[i])
  ...
  plots.ls[[i]] <- ggplot() +
    ...
    theme(plot.title = element_text(size = 12),  # Adjust title size
          legend.position = c(0.95, 0.95),  # Adjust these values to fine-tune position
          legend.justification = c("right", "top"),
          legend.box.just = "right",
          legend.margin = margin(6, 6, 6, 6),
          legend.key.size = unit(0.5, "cm")  # Adjust the size of legend keys if needed
    )
}
```
...
We can also calculate the marginal reliability for every value of theta based on the test information function: 

$\text{marginal reliability} = 1 - \frac{1}{\text{info}(\theta)}$
```{r}
tk07.relplot <- tk07.ti %>% 
  pivot_longer(cols = ends_with("info"), names_to = "group", values_to = "info") %>%
  select(thetas, group, info) %>%
  mutate(group = gsub(".info","",group),
         reliability = 1 - 1/info) %>%
  ggplot() +
  geom_line(aes(x = thetas, y = reliability, color = group)) +
  scale_x_continuous(limits = c(-6, 6)) +
  scale_y_continuous(limits = c(0, 1)) +
  labs(title = "Marginal Reliability of Tests for Grades 3-8",
       x = "Theta",
       y = "Marginal Reliabilty",
       color = "Group",
       fill = "Group") +
  theme(plot.title = element_text(size = 12)) +
  geom_hline(yintercept = 0.7, color = "red", linetype = "dotted") +
  geom_hline(yintercept = 0.8, color = "blue", linetype = "dotted") +
  geom_hline(yintercept = 0.9, color = "green", linetype = "dotted") +
  theme_clean() 

print(tk07.relplot) %>% suppressWarnings()
```
The horizontal dotted red, blue, and green lines represent reliability values of 0.7, 0.8 and 0.9

#### Scaling: Piecewise Linear

Let's take the Piecewise Linear method as before, but first, let's take a look at the distribution of our ability estimates:
```{r}
psych::describe(tk07.abilities$ability_est)
```

Since we have a broader range, let's select more relaxed limits for our theta scale: c(-5, 5). Here is a plot of the scaling.
```{r}
A.piece <- (800 - 200) / (5 - (-5))
...
tk07.scaled.scores %>%
  ggplot(., aes(x = ability_est, y = scale_linear_piece, group = group, color = group)) +
  geom_jitter(alpha = 0.2) +
...
Somewhat expected, we have more 800 scores in higher grades, but interestingly we also have more lower scores in the higher grades. This is likely a result of our simulated data, and 


```{r}
tk07.scaled.scores %>% scale_breakdown(scale_linear_piece)
```
It looks like we have a decent range in the middle quintiles


And now let's visualize student growth over time using the scaled scores.
```{r}
tk07.scaled.scores %>%
  mutate(student = rep(1:1000,6)) %>%
  ggplot(., aes(x = group, y = scale_linear_piece, group = student, color = as.factor(student))) +
  geom_line(alpha = 0.2) +
  labs(
    title = "Piecewise Linear Scaling - Actual Scaled Scores",
    x = "Grade",
    y = "Scaled Score"
  ) +
  theme_clean() +
...
And let's create a simple linear model for each student and examine the growth
```{r}
# Create a function to fit a linear model
fit_linear_model <- function(data) {
  model <- lm(scale_linear_piece ~ timepoint, data = data)
  return(coef(model))
}

tk07.lm <- tk07.scaled.scores %>%
  mutate(timepoint = as.numeric(gsub("g","",group)) - 3) %>%  # Make timepoint start at "0" instead of "3"
  group_by(student) %>%
  nest() %>%
  mutate(fit = map(data, fit_linear_model)) %>%
  unnest_wider(fit) %>%
  select(-data) %>%
  rename_with(~c("student","intercept","slope")) %>% 
  ungroup

tk07.lm %>%
  arrange(slope) %>%
  mutate(g3 = intercept,
         g8 = 5 * slope + intercept,
         student = 1:1000) %>% 
  pivot_longer(cols = c(g3,g8), names_to = "Grade", values_to = "Score") %>%
  ggplot(., aes(x = Grade, y = Score, group = student, color = factor(student))) +
  geom_line(alpha = 0.2) +
  scale_x_discrete(limits = c("g3", "g8"), expand = c(0.01, 0.01)) +  # Control x-axis limits and expansion
  theme_clean() +
    labs(title = "Linear Growth Models of Student Scaled Scores",
       x = "Grade",
       y = "Scaled Score") +
  theme(legend.position = "none")
```

And examine the slope and intercept of those models.
```{r}
tk07.lm %>%
  ggplot(., aes(x = slope, y = intercept)) +
  geom_point(alpha = 0.2, color = "blue3") +
  labs(title = "Slope and Intercept of Student Growth Models",
   x = "Intercept",
   y = "Slope") +
  theme_clean()
```
...
```{r}
tk07.growth.model.1 <- tk07.scaled.scores %>%
  mutate(timepoint = as.numeric(gsub("g","",group)) - 5.5) %>%  # Center the timepoint
  lmer(data = ., 
       scale_linear_piece ~ timepoint + (1 | student))
...
```{r}
tk07.growth.model.2 <- tk07.scaled.scores %>%
  mutate(timepoint = as.numeric(gsub("g","",group)) - 5.5) %>%  # Center the timepoint
  lmer(data = ., 
       scale_linear_piece ~ timepoint + (timepoint | student))
...
tk07.scaled.scores %>%
  mutate(student = rep(1:1000,6)) %>%
  filter(scale_linear_piece == 200) %>%
  arrange(desc(ability_true))
```

```{r}
tk07.responses$responses.8[728,]
est_ability_mle(tk07.responses$responses.8[728,], 
                tk07.pars.out$grade8[,1],
                tk07.pars.out$grade8[,2],
                tk07.pars.out$grade8[,3])
```
...
```{r}
for(i in 1:length(tk07.pars.out)) {
  params <- tk07.pars.out[[i]] %>%
    as.data.frame %>%
    setNames(c("a","b","c")) %>%
    mutate(group = grades[i]) %>%
    select(group, b)
  if(i == 1) {
    tk07.b.params <- params
  } else {
  tk07.b.params <- tk07.b.params %>%
    bind_rows(., params)
  }
}
```
...
```{r}
tk07.b.params %>%
  mutate(item = seq(1:nrow(tk07.b.params)),
         group_position = as.numeric(gsub("g","",group)) - 2) %>%
  ggplot(aes(x = b, y = group_position)) +
  geom_hline(aes(yintercept = group_position), color = "gray80") +
  geom_point(aes(color = group), size = 3, alpha = 0.5) +
  scale_y_continuous(breaks = 1:6, labels = grades) +
  labs(title = "Item Difficulty Parameters Across 6 Grades",
       x = "Item Difficulty (Theta)",
       y = "Grade",
       color = "Grade") +
  theme_clean() +
...
1. Scaling
2. Percentile ranks
3. Visualize the item parameters on the same scale.
4. Longitudinal data simulation
...
## Investigate small ability estimates

```{r}
abilities %>% 
  rownames_to_column("case") %>%
  mutate(case = as.numeric(case),
         case = ifelse(group == "x.new", case - 1000, 
                       ifelse(group == "y", case - 2000,
                              case))) %>% 
  filter(ability_est < -3) %>%
  arrange(ability_est)
```

```{r}
responses.y[c(754, 925),]
```

```{r}
responses.y[579,]

# est_ability(responses.y[579,],
#             as = pars.out$group2[,1],
#             bs = pars.out$group2[,2],
#             cs = pars.out$group2[,3])

abilities %>% 
  ggplot(., aes(x = ability_est, color = group)) +
  geom_histogram()

x.abilities <- x.ability.old %>%
  rename_with(., ~paste0(.x, ".old")) %>%
  bind_cols(x.ability.new %>%
              rename_with(., ~paste0(.x, ".new")))
